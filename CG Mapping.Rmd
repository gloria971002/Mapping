...
#library(viridis)
library(sp)
#"drat","maps","tmap","sf","viridis","sp
```

```{r setup, include=FALSE}
# For states data, transfer the character mode into spatial format
tMap <- st_as_sf(map("county",counties_f,plot=F,fill=T))
# Transfer Floyd_rain into spatial format 
TFloyd <- Floyd_rain %>% 
  select(region,subregion,`Rainfall (mm)`) %>% 
  mutate(ID=str_c(region,subregion,sep = ","))
TFloyd <- left_join(tMap,TFloyd,by="ID")
TFloyd <- TFloyd %>% 
  select(ID,`Rainfall (mm)`, geom) 
# Transfer tTrackFloyd into spatial format
tTrackFloyd=cbind(Floyd_hurr$longitude,Floyd_hurr$latitude)%>%
  Line() %>% Lines(ID='Floyd-1999') %>%
  list() %>% SpatialLines()
# Transfer tTrackFloyd into sf format
tTrackFloyd<- st_as_sf(tTrackFloyd)
```

```{r}
# Mapping
tTrackFloyd %>% 
  sf::st_cast("MULTILINESTRING") -> tTrackFloyd
tm_shape(TFloyd)+
  tm_polygons("Rainfall (mm)",  border.col = "grey",
...
```{r}
# Transfer Allison_rain into spatial format
tAllison <- Allison_rain %>% 
  select(region,subregion,`Rain > 175 mm`) %>% 
  mutate(ID=str_c(region,subregion,sep = ","))
tAllison <- left_join(tMap,tAllison,by="ID")
TAllison <- tAllison %>% 
  select(ID,`Rain > 175 mm`, geom) 
# Transfer tAllison into spatial format
tTrackAllison=cbind(Allison_hurr$longitude,Allison_hurr$latitude) %>%
  Line() %>% Lines(ID='Allison-2001') %>%
  list() %>% SpatialLines()
# Transfer tAllison into sf format
tTrackAllison<- st_as_sf(tTrackAllison)
```
...
# Mapping
tTrackAllison %>% 
  sf::st_cast("MULTILINESTRING") -> tTrackAllison
tm_shape(tAllison)+
  tm_polygons("Rain > 175 mm",  border.col = "grey",
              lwd = 0.1, colorNA = NULL, style="cont",
              title = "Rain > 175 mm",
              palette = "Blues") + 
